...
function parseActivities(lines) {
  const pipeActivities = lines
    ...
    .map((line) => {
      ...
      return { name, role, hoursPerWeek, years, description };
    });
...
async function previewMappedValues() {
  ...
  const values = await getMappedValues(sectionInfo);
  state.lastValues = values;
  renderPreview(values);
  switchTab('fill');
  setStatus(`Previewed ${Object.keys(values).length} mapped fields.`, 'good');
}

async function fillCurrentSection() {
  saveProfile(false);
  const sectionInfo = state.lastSection || await scanVisibleFields();
  const values = state.lastValues || await getMappedValues(sectionInfo);
  const safeValues = filterSensitiveValues(values, sectionInfo);
  const tab = await getActiveUcTab();
  await sendTabMessage(tab.id, { type: 'APPLY_VALUES', values: safeValues });
  renderPreview(safeValues);
  setStatus(`Filled ${Object.keys(safeValues).length} fields on the UC page.`, 'good');
}
...
function renderPreview(values) {
  ...
  $('previewList').innerHTML = entries.map(([id, value]) => `
    <div class="item">
      ...
      <span>${escapeHtml(value === null || value === undefined ? 'Skipped' : String(value))}</span>
    </div>
  `).join('');
}
